# Đánh giá sẵn sàng production — 2026-09-17 (cập nhật 2026-09-18)

**Kết luận: chưa đủ điều kiện triển khai production, nhưng 9/10 điểm chặn gốc đã đóng ở mức code/local, có bằng chứng thực đo trên Docker Desktop local (không chỉ đọc code).** Điểm chặn #10 (Playwright): 3 ca cụ thể đã nêu trong bản gốc đã sửa và pass, nhưng lượt rà soát này cũng phát hiện thêm một vấn đề NGOÀI phạm vi 3 ca đó — chạy lặp lại toàn bộ suite trên cùng một DB dev chưa reset gây lỗi dây chuyền — chưa kết luận được đây có phải vấn đề thật trên CI hay chỉ do cách chạy thủ công lặp lại trong phiên debug này (xem chi tiết ở điểm #10). Còn mở khác: lỗ hổng dependency cần nâng phiên bản NestJS có breaking change, và toàn bộ hạng mục hạ tầng thật (domain, SMTP, TLS cert, staging, backup/restore) — những việc này cần quyết định/thông tin xác thực từ đội vận hành, không thể đóng chỉ bằng sửa code.
...
Trạng thái theo lượt xác minh 2026-09-18. "ĐÃ ĐÓNG (code)" nghĩa là đã sửa và kiểm chứng được trong repository/local Docker; triển khai thật lên staging/production ở bước sau vẫn cần chạy lại trên hạ tầng thật.

1. **✅ ĐÃ ĐÓNG (code).** Đường truy cập production. `docker-compose.prod.yml` nay có service `web` (Nginx, port 80/443, TLS cert volume) proxy `/api/` sang `backend:3000` qua Docker network nội bộ; backend không publish port ra host. Đã build thử cả 3 image (`runner`, `migrate`, frontend) bằng Docker Desktop local — build thành công đúng như job `production-images` trong CI.
...
6. **✅ ĐÃ ĐÓNG (code + xác minh thật).** Image `migrate` build từ stage `builder` (trước khi prune dev deps) nên có đủ Prisma CLI và ts-node khóa đúng phiên bản lockfile — đã xác nhận `require('prisma')`/`require('ts-node')` chạy được bên trong image vừa build. Migration 016/017 đã chạy qua `npm run test:isolated` (67/67 pass); chưa chạy trên DB production thật vì DB đó chưa tồn tại.
...
- `.github/workflows/ci.yml` có lint/typecheck/unit/build + 2 job mới (`postgres-api`, `production-images`) build container và chạy test isolated trên PostgreSQL thật; job này build/run local thành công ở lượt 2026-09-18 nhưng **chưa chạy remote trên GitHub Actions** — môi trường runner GitHub có thể khác máy local (network, quyền, phiên bản Docker).
- Docker Desktop local đã phục hồi và build/run thành công cả 3 image production (`runner`, `migrate`, frontend) — xem điểm chặn #1, #5, #6.
