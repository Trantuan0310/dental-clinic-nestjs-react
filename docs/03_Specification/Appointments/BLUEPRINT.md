...
| BR-APPT-012 | Auto no-show: status `scheduled`/`confirmed` mà quá 30 phút sau startAt (hết cửa sổ check-in) → tự động no_show |
...
| BN quên check-in | Cron job auto no-show sau 30 phút (khi cửa sổ check-in đóng) |
