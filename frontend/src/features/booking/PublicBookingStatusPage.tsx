...
export default function PublicBookingStatusPage() {
  ...
  const load = async (ref = reference, access = token) => {
    ...
    try {
      ...
      setStatus(null);
      setError(bookingErrorMessage(e, "Không tìm thấy yêu cầu hoặc mã tra cứu không đúng."));
    } finally {
      setBusy(false);
    }
  };
  ...
  const action = async (path: string, body?: unknown) => {
    ...
    try {
      ...
      setError(bookingErrorMessage(e, "Không thực hiện được thao tác."));
    } finally {
      setBusy(false);
    }
  };
  ...
  const submitDetails = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(bookingErrorMessage(e, "Không cập nhật được thông tin."));
    } finally {
      setBusy(false);
    }
  };
